import { getRequestList, getRequestSingle, GetRequestResultList, GetRequestResultSingle } from './getRequest';
...
describe('Get Request Methods', () => {
  ...
  describe('getRequestList', () => {
    it('calls fetchWithAuth with correct parameters for list request', async () => {
      const mockResponse: GetRequestResultList<{ id: number; name: string }> = {
    ...
    it('works with different endpoint values', async () => {
      const mockResponse: GetRequestResultList<{ id: number }> = {
        data: [{ id: 1 }],
        responseMessage: 'Success',
        successful: true,
      };
  ...
  describe('getRequestSingle', () => {
    it('calls fetchWithAuth with correct parameters for single request', async () => {
      const mockResponse: GetRequestResultSingle<{ id: number; name: string }> = {
    ...
    it('works with different endpoint values', async () => {
      const mockResponse: GetRequestResultSingle<{ id: number }> = {
        data: { id: 999 },
        responseMessage: 'Success',
        successful: true,
      };
